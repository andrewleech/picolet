# Frozen-manifest baseline for the picolet `mcp` variant.
#
# Identical to manifest_cli.py: the mcp variant is the cli baseline plus
# TLS (Q1 DECISION).  The claude-net-mpy plugin libraries (mpyws, the
# hub client, etc.) ship via the app's own romfs at build time (Q4
# DECISION), not as frozen modules in this runtime manifest — this file
# stays minimal.
#
# Pulls in:
