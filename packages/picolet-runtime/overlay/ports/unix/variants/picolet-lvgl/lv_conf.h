/**
 * @file lv_conf.h
 * picolet runtime, lvgl variant.  Configuration file for LVGL v9.3.0.
 *
 * PICOLET_LVGL_CONFIG: this token identifies the file as the picolet
 * overlay's hand-tuned config, distinguishing it from the upstream
 * lv_binding_micropython default at overlay/lib/lv_binding_micropython/
 * lv_conf.h (which is part of the pinned submodule and is not modified).
 *
 * The picolet-lvgl variant .mk passes -DLV_CONF_PATH=<this-file> to point
 * the LVGL build at this tuned config.
 *
 * Tuning targets NFR-3 (lvgl variant <= 2 MiB).  Disabled features:
 *   - all widgets except LABEL, BUTTON, IMAGE, plus OBJ/SCR (mandatory)
 *   - calendar, chart, canvas, table, keyboard, list, msgbox, etc.
 *   - all themes except DEFAULT (needed for any widget to draw)
 *   - file system drivers (FS_STDIO, FS_POSIX, FS_FATFS, etc.)
 *   - LV_USE_LODEPNG, LV_USE_TJPGD, LV_USE_GIF, LV_USE_QRCODE,
 *     LV_USE_BARCODE — image decoders not needed for hello-world
 *   - LV_USE_OBSERVER, LV_USE_IMGFONT, LV_USE_SNAPSHOT
 *   - demos
 *
 * Re-enabling: change the relevant LV_USE_* below from 0 to 1 and
 * record the size impact in a [PH11] Note commit.
 */
...
/* PH11 / NFR-3: only Montserrat 14 (the default font).  Each font bitmap
 * costs 5-15 KB; we ship one. */
...
/* PH11 / NFR-3 widget set: only OBJ (always on), LABEL, BUTTON, IMAGE.
 * Re-enable individually with a [PH11] Note commit recording the size delta. */
