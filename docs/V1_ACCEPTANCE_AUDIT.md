...
| FR-CLI-4 | No `--target` → builds for host. | **Yes** | `build_cmd.py:157` (`target = args.target if args.target else _host_target()`); `_host_target()` at `build_cmd.py:336-354`. |
...
| FR-CLI-7 | `picolet dev` watches and re-runs `build`+`run` on UI-asset or Python-source change; no live-reload of Python state. | **Yes** | `packages/picolet-cli/picolet/dev_cmd.py:67-188` watch loop; `_collect_watch_paths` (from `run_cmd.py:139-157`) watches `src/`, `picolet.toml`, `ui` directory; `_kill_child` (`dev_cmd.py:109-120`) terminates and respawns the child rather than hot-reloading state. |
...
| ID | Requirement (paraphrase) | Verdict | Evidence |
|---|---|---|---|
| FR-BP-1 | `picolet build` resolves variant from `[ui] renderer` (absent → cli) and target from `--target` or host. | **Yes** | `build_cmd.py:136-152` (variant resolution; absent `[ui]` → `cli`); `build_cmd.py:157` (target = `args.target or _host_target()`). |
...
|---|---|---|---|
| NFR-1 | `picolet-runtime-{target}-cli` ≤ 1 MB. | **Yes** | linux-x64-cli: 641,328 B (61.2% of 1,048,576 B); windows-x64-cli: 578,048 B (55.1%). Inspected via `ls -la packages/picolet-runtime/build/`. |
| NFR-2 | `picolet-runtime-{target}-webview` ≤ 2 MB (excluding system webview). | **Yes** | linux-x64-webview: 678,192 B (32.3% of 2,097,152 B); windows-x64-webview: 660,480 B (31.5%). The system webview (WebKitGTK on Linux, Edge WebView2 runtime on Windows) is dynamic and excluded per spec. |
| NFR-3 | `picolet-runtime-{target}-lvgl` ≤ 2 MB. | **Yes** | linux-x64-lvgl: 1,646,952 B (78.5%); windows-x64-lvgl: 2,078,208 B (99.1% — razor-thin, see carryover §4). Both under ceiling. |
...
| NFR-6 | All Picolet-authored code is MIT. | **Yes** | Root `LICENSE` is MIT. Spot-check of file headers (e.g. `overlay/ports/unix/main.c:1-26`, `overlay/ports/unix/variants/picolet-cli/mpconfigvariant.h:1-25`, `python/picolet/_dispatcher.py:1-18`, `bridge-js/src/index.ts`) — all MIT or upstream MicroPython-MIT. No GPL/AGPL claims in picolet-authored code (full-tree grep over `packages/{picolet-cli,picolet-bridge-js,picolet-templates,picolet-runtime/python,picolet-runtime/overlay}` returned only one hit: the `LGPL-2.1-or-later` *allowlist string* in `sbom_gen.py:76`). |
...
| 9 | Windows LVGL margin at 99.1% of NFR-3 (2,078,208 / 2,097,152 = 99.1%; 18,944 bytes headroom). | **Accept for v1.0 with explicit warning**. | The ceiling is met. The 18 KB headroom is razor-thin and a single additional LVGL widget or SDL2 function pulled in by future Python code could blow the gate. Action item for v1.1: either raise NFR-3 on Windows (because static SDL2 + .pdata unwind tables dominate the budget and are not picolet-controllable) or invest in further dead-code elimination. **Recommend release notes flag this explicitly.** |
